...
void display(void) {
    ...
    modelview = modelview
                * glm::translate(identity, glm::vec3(-0.4, -0.4, 0.0));
    glUniformMatrix4fv(modelviewPos, 1, GL_FALSE, &(modelview)[0][0]);
    ...
    pushMatrix(modelview);
    modelview = modelview
                * glm::translate(identity, glm::vec3(teapotloc, 0.0, 0.0));
    ...
    modelview = modelview * glm::rotate(glm::mat4(1.0f),
                                        rotamount * glm::pi<float>() / 180.0f,
...
void checkOpenGLVersion(void) {
    ...
    if (version_p != NULL) version = atof(version_p);

    if (version < 3.1f) {
        std::cout << "\n*****************************************\n";

        if (version_p != NULL) {
            std::cout << "WARNING: Your OpenGL version is not supported.\n";
            std::cout << "We detected version " << std::fixed
                      << std::setprecision(1) << version;
            std::cout << ", but at least version 3.1 is required.\n\n";
        } else {
            std::cout
                << "WARNING: Your OpenGL version could not be detected.\n\n";
        }

        std::cout << "Please update your graphics drivers"
                  << " BEFORE posting on the forum. If this\n"
                  << "doesn't work, ensure your GPU supports OpenGL 3.1"
                  << "or greater.\n";

        std::cout << "If you receive a 0xC0000005: Access Violation error,"
                  << " this is likely the reason.\n";

        std::cout << std::endl;

        std::cout << "Additional OpenGL Info:\n";
        std::cout << "(Please include with support requests)\n";
        std::cout << "GL_VERSION: ";
        ...
        std::cout << "\n*****************************************\n";
        std::cout << "\nSelect terminal and press <ENTER> to continue.\n";
        std::cin.get();
        std::cout << "Select OpenGL window to use commands below.\n";
    }
}
