    ...
    if (in.is_open()) {
        ...
        while (in) {
            if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
                ...
                string str[10];  // store a list of C++ string, just like values but strings
                int numverts = 0;
                ...
                if (cmd == "directional" || cmd == "point") {
                    if (numused == numLights) { // no more lights
                        ...
                        values[7] = 1.0f; // assume alpha = 1
                        for (i = 6; i > 3; i--) values[i] = values[i - 1];
                        if (cmd == "directional") {
                            values[3] = 0;
                        } else {
                            values[3] = 1;
                ...
                // Note that no transforms/stacks are applied to the colors.
                    ...
                    if (numverts > maxverts) {
                        ...
                        validinput = readvals(s, 3, values);
                        if (validinput) vertex[numverts] = vec3(values[0], values[1], values[2]);
                        ++numverts;
                    }
                }
                ...
                // read the max number of bounces for a ray
                else if (cmd == "maxdepth") {
                    validinput = readvals(s, 1, values);
                    if (validinput) maxdepth = values[0];
                }

                // the output file to which the images should be written
                else if (cmd == "output") {
                    validinput = readstr(s, 1, str);
                    if (validinput) outfile = str[0];
                }
