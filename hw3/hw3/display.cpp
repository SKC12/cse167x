...
void display() {
    ...
    if (numused) {
        ...
        GLfloat lightSetOfFour[4];
        for (int i = 0; i < numused * 4; i += 4) {
            // extract each set of 4
            lightSetOfFour[0] = lightposn[i];
            lightSetOfFour[1] = lightposn[i + 1];
            lightSetOfFour[2] = lightposn[i + 2];
            lightSetOfFour[3] = lightposn[i + 3];
            // transform the set of 4
            transformvec(lightSetOfFour, lightSetOfFour);
            // save output to lightransf at the exact index
            lightransf[i] = lightSetOfFour[0];
            lightransf[i + 1] = lightSetOfFour[1];
            lightransf[i + 2] = lightSetOfFour[2];
            lightransf[i + 3] = lightSetOfFour[3];
        }
    ...
    mat4 sc(1.0f), tr(1.0f), transf(1.0f);
    sc = Transform::scale(sx, sy, 1.0f);
    tr = Transform::translate(tx, ty, 0.0f);
